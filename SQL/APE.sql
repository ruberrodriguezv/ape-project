...
--
-- Temporary table structure for view `producto_precio_stock`
--

DROP TABLE IF EXISTS `producto_precio_stock`;
/*!50001 DROP VIEW IF EXISTS `producto_precio_stock`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `producto_precio_stock` (
  `id_producto` int(11),
  `nombre` varchar(45),
  `valor` float unsigned zerofill,
  `cantidad` int(10) unsigned zerofill
) ENGINE=MyISAM */;
SET character_set_client = @saved_cs_client;

--
-- Final view structure for view `producto_precio_stock`
--

/*!50001 DROP TABLE IF EXISTS `producto_precio_stock`*/;
/*!50001 DROP VIEW IF EXISTS `producto_precio_stock`*/;
...
/*!50001 VIEW `producto_precio_stock` AS select `a`.`id_producto` AS `id_producto`,`a`.`nombre` AS `nombre`,`b`.`valor` AS `valor`,`c`.`cantidad` AS `cantidad` from ((`producto` `a` left join `precio_producto` `b` on((`a`.`id_producto` = `b`.`id_producto`))) left join `stock` `c` on((`a`.`id_producto` = `c`.`id_producto`))) where (`b`.`fecha_creacion` = (select max(`precio_producto`.`fecha_creacion`) from `precio_producto` where (`precio_producto`.`id_producto` = `a`.`id_producto`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Dumping routines for database 'ape'
--
...
-- Dump completed on 2011-06-16  1:19:59
